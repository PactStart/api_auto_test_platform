...
exports.addAppConfig = (req, res) => {
  ...
  const selectSql =
    "select * from app_config where app_id = ? and config_key = ?";
  db.query(selectSql, [appId, configKey], (err, results) => {
    ...
    if (results && results.length) {
      const updateSql =
        "update app_config set del = 0,config_value = ? where app_id = ? and config_key = ?";
    ...
    const now = Date.now();
    const insertSql =
      "insert app_config(app_id,config_key,config_value,description,create_at,create_by,update_at) values(?,?,?,?,?,?,?)";
    db.query(
      insertSql,
      [
        appId,
...
exports.deleteAppConfig = (req, res) => {
  ...
  const currentUser = parseToken(req);
  const deleteSql =
    "update app_config set del = 1,update_at = ?, update_by = ? where id = ?";
  db.query(
    deleteSql,
...
exports.queryAppConfig = (req, res) => {
  ...
  //查询app配置列表sql
  const pageSql =
    "select * from app_config " + whereSql + " order by id limit ?,?";
