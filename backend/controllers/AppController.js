...
exports.addApp = (req,res) => {
    ...
    db.query(selectSql,name,(err,results) => {
        ...
        const now = Date.now();
        const insertSql = 'insert app(name,create_at,create_by,update_at,update_by) values(?,?,?,?,?)';
        db.query(insertSql,[name,now,currentUser.nickname,now,''],(err,results) => {
            if(err) {
                return res.send({
                    code: 1,
                    msg: err.message
...
exports.deleteApp = (req,res) => {
    let {id} = req.body;
    const deleteSql = 'update app set del = 1,update_at = ?,update_by = ? where id = ?';
...
exports.updateApp = (req,res) => {
    ...
    const currentUser = parseToken(req);
    const updateSql = 'update app set name = ?,update_at = ?,update_by = ? where id = ?';
    db.query(updateSql,[name,id,Date.now(),currentUser.nickname],(err,results) => {
        if(err) {
            return res.send({
                code: 1,
                msg: err.message
