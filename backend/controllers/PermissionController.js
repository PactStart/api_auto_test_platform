...
exports.deletePermission = (req,res) => {
    ...
    db.query(selectSql,id,(err,results) => {
        ...
        if(results && results.length) {
            ...
            const currentUser = parseToken(req);
            const now = Date.now();
            const deleteSql = 'update sys_permission set del = 1, update_at= ?, update_by = ? values (?,?,?) where id = ?';
        ...
        db.getConnection((err,connection) => {
            ...
            connection.beginTransaction(function(err) {
                ...
                for(let apiGroup of groupArr) {
                    ...
                    connection.query(insertGroupSql,[0,'API',0,apiGroup.name,apiGroup.description,0,now,currentUser.nickname,now],(err,groupInsertResults) => {
                        if(err) {
                            return res.send({
                                code: 1,
                                msg: err.message
                        ...
                        for(let apiModule of apiGroup.children) {
                            ...
                            connection.query(insertModuleSql,[groupId,'API',0,apiModule.name,apiModule.description,0,now,currentUser.nickname,now],(err,moduleInsertResults) => {
                                if(err) {
                                    return res.send({
                                        code: 1,
                                        msg: err.message
                                ...
                                for (let api of apiModule.children) {
                                    insertSql = insertSql + `(${moduleRecordId},'API',0,'${api.apiUrl}','${api.apiName}',0,${now},'${currentUser.nickname}',${now}),\r\n`;
                                }
