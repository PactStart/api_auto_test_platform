    ...
    db.query(selectSql,name,(err,results) => {
        ...
        if(results && results.length) {
            if(!results[0].del) {
                ...
                const now = Date.now();
                const updateSql = 'update sys_role set del = 0, update_at= ?, update_by = ? where id = ?';
                db.query(updateSql,[now,currentUser.nickname,results[0].id],(err,results) => {
                    if(err) {
                        res.send({
                            code: 1,
                            msg: err.message
            ...
            const currentUser = parseToken(req);
            const now = Date.now();
            const insertSql = 'insert into sys_role(name,description,create_at,create_by,update_at) values (?,?,?,?,?)';
            db.query(insertSql,[name,description,now,currentUser.nickname,now],(err,results) => {
                if(err) {
                    res.send({
                        code: 1,
                        msg: err.message
...
exports.deleteRole= (req,res) => {
    ...
    const currentUser = parseToken(req);
    const now = Date.now();
    const deleteSql = 'update sys_role set del = 1, update_at= ?, update_by = ? where id = ?';
...
exports.updateRole = (req,res) => {
    ...
    const currentUser = parseToken(req);
    const now = Date.now();
    const updateSql = 'update sys_role set description = ?, update_at= ?, update_by = ? where id = ?';
    db.query(updateSql,[description, now, currentUser.nickname,id],(err,results) => {
        if(err) {
            return res.send({
                code: 1,
                msg: err.message
    ...
    db.getConnection((err,connection) => {
        ...
        connection.beginTransaction(function(err) {
            ...
            connection.query(deleteSql, roleId, (err,results) => {
                ...
                const currentUser = parseToken(req);
                const now = Date.now();
                let insertSql = 'insert sys_role_permission(role_id,permission_id,create_at,create_by,update_at) values';
                for (let index = 0; index < permissionIds.length; index++) {
                    const permissionId = permissionIds[index];
                    insertSql = insertSql + `(${roleId},${permissionId},${now},'${currentUser.nickname}',${now}),`;
                }
...
exports.bindPermissions = (req,res) => {
    ...
    db.query(selectSql,(err,results) => {
        ...
        insertSql = insertSql.substring(0,insertSql.length-1);
        insertSql = insertSql + `on duplicate key update update_at=values(update_at),update_by='${currentUser.nickname}'`
...
exports.listPermissions = (req,res) => {
    ...
    const pageSql = 'select p.id,parent_id,type,anon,name,description,internal from sys_permission p join sys_role_permission rp on p.id = rp.permission_id where rp.del = 0 and rp.role_id = ? order by rp.id desc limit ?,?';
    const totalSql = 'select count(1) as total from sys_permission p join sys_role_permission rp on p.id = rp.permission_id where rp.del = 0 and rp.role_id = ? ';
...
exports.assignToUsers = (req,res) => {
    ...
    const currentUser = parseToken(req);
    const now = Date.now();
    let insertSql = 'insert sys_user_role(user_id,role_id,create_at,create_by,update_at) values';
    for (let index = 0; index < userIds.length; index++) {
        const userId = userIds[index];
        insertSql = insertSql + `(${userId},${roleId},${now},'${currentUser.nickname}',${now}),`;
    }
    insertSql = insertSql.substring(0,insertSql.length-1);
    db.query(insertSql,(err,results) => {
        if(err) {
...
exports.bindUsers = (req,res) => {
    ...
    insertSql = insertSql.substring(0,insertSql.length-1);
    insertSql = insertSql + `on duplicate key update update_at=values(update_at),update_by=values(create_by)`
    db.query(insertSql,(err,results) => {
        if(err) {
            res.send({
                code: 1,
...
exports.unbindUsers = (req,res) => {
    let {roleId,userIds} = req.body;
    const deleteSql = 'delete from sys_user_role where role_id = ? and user_id in (?)';
...
exports.listUsers = (req,res) => {
    ...
    const pageSql = 'select u.id,u.nickname,u.username,u.phone,u.email from sys_user u join sys_user_role ur on u.id = ur.user_id where ur.del = 0 and ur.role_id = ? order by ur.id desc limit ?,?';
    const totalSql = 'select count(1) as total from sys_user u join sys_user_role ur on u.id = ur.user_id where ur.del = 0 and ur.role_id = ?';
