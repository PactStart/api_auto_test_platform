...
exports.loginByUserName = (req,res) => {
    ...
    db.query(userSelectSql,username,(err,results) => {
        ...
        const now = Date.now;
        const updateSql = 'update user set last_login_ip = ? , last_login_time = ? where id = ?';
...
exports.userInfo = (req,res) => {
  ...
  const roleSelectSql = 'select r.* from sys_user_role ur join sys_role r on ur.role_id = r.id where ur.user_id = ?';
  db.query(roleSelectSql,currentUser.id,(err,results) => {
    if(!err && results && results.length) {
        ...
        const permissionSelectSql = 'select p.type,p.name from sys_role_permission rp join sys_permission p on rp.permission_id = p.id where rp.role_id in (?)';
...
exports.resetPwd = (req,res) => {
    ...
    db.query(selectSql,currentUser.id,(err,results) => {
        ...
        const updateSql = 'update sys_user set password = ?, update_at = ?, update_by = ? where id = ?';
        db.query(updateSql,[passwordB,currentUser.id,Date.now(),currentUser.nickname],(err,results) => {
            if(err) {
                return res.send({
                    code: 1,
                    msg: err.message
...
exports.addUser = (req,res) => {
    ...
    db.query(selectSql,username,(err,results) =>{
        ...
        const currentUser = parseToken(req);
        const now = Date.now();
        const insertSql = 'insert into sys_user(username,password,nickname,create_at,create_by,update_at,update_by) values(?,?,?,?,?,?,?)';
        db.query(insertSql,[username,passwordB,nickname,now,currentUser.nickname,now,''],(err,results) => {
            if (err) {
                return res.send({ 
                    code: 1, 
                    msg: err.message 
...
exports.updateUser = (req,res) => {
    ...
    if(currentUser.id === id || currentUser.super_admin) {
        const updateSql = 'update sys_user set phone = ?,email = ?,update_at = ? ,update_by = ? where id = ?';
        db.query(updateSql,[phone,email,Date.now(),currentUser.nickname,id],(err,results) => {
            if (err) {
                return res.send({ 
                    code: 1, 
                    msg: err.message 
...
exports.queryUser = (req,res) => {
    ...
    if(keyword) {
        keyword = '%'+keyword+'%';
        pageSql = 'select * from sys_user where del=0 and (nickname like ? or username like ? or phone like ? or email like ? ) order by id limit ?,?';
        totalSql = 'select count(*) as total from sys_user where del=0 and (nickname like ? or username like ? or phone like ? or email like ? )';
        params = [keyword,keyword,keyword,keyword];
    }
    db.query(pageSql, [...params, Number(page), Number(size)], (err, results1) => {
        if (err) {
            return res.send({ 
                code: 1,
                msg: err.message 
